...
const FIX =
  "Add it to .env.local in the project root, then stop and restart the dev server — Next.js only reads .env.local at startup.";
...
export function supabaseAnonKey(): string {
  ...
  if (!value) missing("NEXT_PUBLIC_SUPABASE_ANON_KEY");
  return value;
}
...
export function supabaseServiceRoleKey(): string {
  ...
  if (!value) missing("SUPABASE_SERVICE_ROLE_KEY");
  return value;
}
