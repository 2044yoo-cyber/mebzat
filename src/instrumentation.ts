...
export async function register() {
  ...
  const { validateAll } = await import("@/lib/ai/provider-health");
  const { IMAGE_PROVIDERS } = await import("@/lib/ai/image-models");
  const { isUsable, PROVIDER_STATUS } = await import(
    "@/lib/ai/provider-status"
  );

  void validateAll({ force: true })
    .then((health) => {
      const ok = health.filter((entry) => isUsable(entry.status));
      const configured = health.filter(
        (entry) => entry.status !== "missing_key",
      );
      ...
      for (const entry of configured) {
        ...
        console.log(
          `[medosha-ai] ${mark} ${label}: ${status}${entry.reason && !isUsable(entry.status) ? ` — ${entry.reason}` : ""}`,
        );
      }

      if (configured.length === 0) {
        console.log(
          "[medosha-ai] No image provider configured. Set FAL_KEY, TOGETHER_API_KEY or HUGGINGFACE_API_KEY in .env.local — all three have a free tier.",
        );
      } else if (ok.length === 0) {
        console.log(
          "[medosha-ai] No image provider passed validation. Image generation will report which key to fix.",
        );
      }
    })
    .catch((error) => {
